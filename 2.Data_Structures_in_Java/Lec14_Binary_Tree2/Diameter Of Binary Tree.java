...
public class Solution {
    ...
    public static Pair<Integer,Integer> heightdiameter(BinaryTreeNode<Integer> root){
        if(root==null){
            Pair<Integer,Integer> hd=new Pair<>();
            hd.height=0;
            hd.diameter=0;
            return hd;
        }
        ...
        int heights=1+Math.max(lo.height,ro.height);
        int diameters=1+Math.max(lo.height+ro.height,Math.max(lo.diameter,ro.diameter));
